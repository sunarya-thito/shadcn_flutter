  ...
  for (final entity in libDir.listSync(recursive: true)) {
    ...
    for (final decl in unit.declarations) {
      if (decl is ClassDeclaration) {
        ...
        final extendedTypeSource = decl.onClause?.extendedType.toSource();
        topics.add(
          _Topic(
            slug:
                'extension-${name ?? 'on-${(extendedTypeSource ?? 'unknown').replaceAll(' ', '-')}'}',
            title: 'Extension: ${name ?? extendedTypeSource ?? 'unknown'}',
...
List<_Topic> _collectExampleTopics() {
  final topics = <_Topic>[];
  final examplesRoot = Directory('packages/docs/lib/pages/docs/components');
