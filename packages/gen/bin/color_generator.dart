...
void main() {
  ...
  for (var color in colors.entries) {
    ...
    if (value is List) {
      // multi shade color
      Map<int, String> shades = {};
      for (final val in value) {
        final scale = val['scale'];
        var hex = val['hex'].toString();
        if (hex.startsWith('#')) {
          hex = hex.substring(1);
        }
        if (hex.length == 6) {
      ...
      result += "\t});\n";
      primaryNames.add(color.key);
    } else if (value is Map) {
      // single color
      var hex = value['hex'].toString();
      if (hex.startsWith('#')) {
        hex = hex.substring(1);
      }
      if (hex.length == 6) {
  ...
  // add transparent
  result += "\tstatic const Color transparent = Color(0x00000000);\n";
  // add primaries
  result += "\tstatic final List<Color> primaries = [\n";
  for (var name in primaryNames) {
    result += "\t\t$name,\n";
  }
  result += "\t];\n";
  result += "}\n";
  File('packages/shadcn_flutter/lib/src/theme/generated_colors.dart').writeAsStringSync(result);
  print('done');
}
