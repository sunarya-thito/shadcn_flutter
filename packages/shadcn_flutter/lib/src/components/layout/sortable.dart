...
class _SortableState<T> extends State<Sortable<T>>
  ...
  void _onDragStart(DragStartDetails details) {
    ...
    final customGhost = widget.ghost;
    ...
    _session = _SortableDraggingSession(
      ...
      transform: transform,
      size: size,
      // When the ghost falls back to widget.child (no custom ghost/candidate
      // widget given), it must reuse `_key` so the same Element - and thus
      // its live State (e.g. toggled colors) - follows the cursor instead of
                  ...
                  Flexible(
                    child: Column(
                      ...
                      children: [
                        ...
                        Flexible(
                          // While dragging, `_key` lives on the ghost copy
                          // (see _onDragStart) so this hidden placeholder
                          // must render an unkeyed copy of widget.child -
                          // reusing `_key` here too would claim it twice in
                          // the same frame and throw a duplicate GlobalKey
                          // error. Its state doesn't matter since it stays
                          ...
                          child: _dragging
                              ...
                              : ListenableBuilder(
                                  ...
                                  builder: (context, child) {
                                    // Once a drop is claimed, `_key` moves to
                                    // the flying drop-transform ghost in
                                    // _SortableLayerState (see _claimDrop),
                                    // which reuses this session's ghost
                                    // widget. Keeping `_key` here too would
                                    // claim it twice in the same frame.
