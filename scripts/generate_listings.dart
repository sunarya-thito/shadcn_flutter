...
void main() async {
  ...
  final iconFiles = iconDir.listSync().whereType<File>().where((f) => f.path.endsWith('.dart'));
  for (final file in iconFiles) {
    final fileName = p.basenameWithoutExtension(file.path);
    final className = '${fileName.split('_').map((s) => s[0].toUpperCase() + s.substring(1)).join()}s';
    final setName = fileName.split('_').map((s) => s[0].toUpperCase() + s.substring(1)).join(' ');
  ...
  colorBuffer.writeln();
  colorBuffer.writeln('Shadcn Flutter uses a Tailwind-inspired color palette and a flexible theme system.');
  colorBuffer.writeln();

  if (colorsFile.existsSync()) {
    colorBuffer.writeln('## Color Palette');
    colorBuffer.writeln('Access via `Colors.colorName.shade` (e.g., `Colors.blue[500]`).');
    colorBuffer.writeln();
    
    final result = parseFile(path: colorsFile.absolute.path, featureSet: featureSet);
    final unit = result.unit;
    for (final declaration in unit.declarations) {
      if (declaration is ClassDeclaration && declaration.name.lexeme == 'Colors') {
        ...
        for (final member in declaration.members) {
          if (member is FieldDeclaration && member.isStatic) {
            ...
            if (type == 'ColorShades') {
              for (final field in member.fields.variables) {
                colorBuffer.writeln('| **${field.name.lexeme}** | 50, 100, 200, 300, 400, 500, 600, 700, 800, 900, 950 |');
              }
            }
          }
        }
  ...
  if (themesFile.existsSync()) {
    ...
    colorBuffer.writeln('Usage: `ShadcnApp(theme: ThemeData(colorScheme: ColorSchemes.schemeName))`');
    colorBuffer.writeln();

    final result = parseFile(path: themesFile.absolute.path, featureSet: featureSet);
    final unit = result.unit;
    for (final declaration in unit.declarations) {
      if (declaration is ClassDeclaration && declaration.name.lexeme == 'ColorSchemes') {
        final schemes = <String>[];
        for (final member in declaration.members) {
          if (member is FieldDeclaration && member.isStatic && member.fields.type?.toString() == 'ColorScheme') {
            for (final field in member.fields.variables) {
